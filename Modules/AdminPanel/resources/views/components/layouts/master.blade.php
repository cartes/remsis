...
<body class="font-sans antialiased bg-slate-50 flex overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <div class="h-20 flex items-center px-6 border-b border-gray-100 flex-shrink-0">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-indigo-500/20">
                        R
                    </div>
                    <span class="font-black text-slate-800 tracking-tighter text-2xl uppercase">Remsis</span>
                </a>
            </div>
    ...
    <div x-data x-show="$store.toast?.show" x-transition.opacity
        ...
        <div class="rounded-lg shadow-lg px-4 py-3 text-sm flex items-start gap-3 border-2"
            :class="{
                'bg-green-600 text-white border-green-400': $store.toast.type==='success',
                'bg-red-600 text-white border-red-400': $store.toast.type==='error',
                'bg-blue-600 text-white border-blue-400': $store.toast.type==='info'
            }"
            role="status" aria-live="polite">
            <div class="pt-0.5">
                <template x-if="$store.toast.type==='success'"><i class="fas fa-check-circle text-lg"></i></template>
                <template x-if="$store.toast.type==='error'"><i
                        class="fas fa-circle-exclamation text-lg"></i></template>
                <template x-if="$store.toast.type==='info'"><i class="fas fa-circle-info text-lg"></i></template>
            </div>
            <div class="whitespace-pre-line font-bold" x-text="$store.toast.message"></div>
        ...
        (function() {
            function registerStore() {
                Alpine.store('toast', {
                    ...
                    type: 'success',
                    _timer: null,
                    flash(message, type = 'success', timeout = 4000) {
