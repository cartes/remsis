<x-guest-layout>
    <div class="mb-8 text-center">
        <h2 class="text-2xl font-black text-slate-800 tracking-tight font-outfit">Crea tu Cuenta</h2>
        <p class="text-slate-500 text-sm mt-2 font-medium">Únete a la gestión laboral inteligente.</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-6">
        ...
        <div class="space-y-2">
            <label for="name"
                class="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 px-1">
            ...
            <div class="relative group">
                ...
                <div
                    class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-slate-300 group-focus-within:text-indigo-500 transition-colors">
                    <i class="fas fa-signature"></i>
                </div>
            </div>
            <x-input-error :messages="$errors->get('name')" class="mt-2 text-[11px] font-bold uppercase tracking-tight" />
        </div>
        ...
        <div class="space-y-2">
            ...
            <div class="relative group">
                <input id="email" type="email" name="email" value="{{ old('email') }}" required
                    autocomplete="username" placeholder="ejemplo@remsis.cl"
                    class="w-full px-5 py-4 bg-slate-50/50 border border-slate-200 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all outline-none">
                <div
                    class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-slate-300 group-focus-within:text-indigo-500 transition-colors">
                    <i class="fas fa-at"></i>
