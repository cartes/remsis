<x-adminpanel::layouts.master>
    ...
    @section('content')
        ...
        <div x-data="{ activeTab: 'resumen', dropdownItemsOpen: false }" @click.away="dropdownItemsOpen = false" class="min-h-screen bg-slate-50">
                ...
                <div class="max-w-7xl mx-auto px-6 py-6">
                    <div class="mb-4">
                        <x-breadcrumb :items="[
                            ...
                            [
                                'label' => $employee->company->razon_social ?? $employee->company->name,
                                'url' => route('companies.dashboard', $employee->company),
                            ],
                            ['label' => 'Colaboradores', 'url' => route('companies.employees', $employee->company)],
                    ...
                    <div class="flex items-center gap-5">
                        ...
                        <div class="relative flex-shrink-0">
                            ...
                            @else
                                <div
                                    class="w-[72px] h-[72px] rounded-2xl bg-gradient-to-br from-slate-700 to-slate-900 flex items-center justify-center ring-2 ring-slate-100 select-none">
                            ...
                            @endif
                            <span
                                class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white {{ ($emp?->status ?? 'active') === 'active' ? 'bg-emerald-500' : 'bg-slate-300' }}"></span>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2.5 flex-wrap">
                                ...
                                @if ($emp?->rut)
                                    <span
                                        class="text-xs font-mono bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md">{{ $emp->rut }}</span>
                                @endif
                                <span
                                    class="text-xs font-semibold px-2 py-0.5 rounded-full {{ ($emp?->status ?? 'active') === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200' }}">
                        ...
                        <div class="hidden sm:flex flex-col items-end gap-1.5 flex-shrink-0">
                            <span
                                class="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Completitud</span>
                            ...
                            {{ $completion >= 80 ? 'bg-emerald-500' : ($completion >= 50 ? 'bg-amber-400' : 'bg-red-400') }}"
                                ...
                                <span
                                    class="text-sm font-bold {{ $completion >= 80 ? 'text-emerald-600' : ($completion >= 50 ? 'text-amber-600' : 'text-red-500') }}">{{ $completion }}%</span>
                            </div>
                        </div>
                        ...
                        <nav class="flex items-stretch gap-0 -mb-px overflow-x-auto scrollbar-hide">
                            ...
                            <div class="relative flex items-stretch">
                                <button @click="dropdownItemsOpen = !dropdownItemsOpen"
                                    ...
                                    <span
                                        :class="['haberes', 'descuentos', 'creditos'].includes(activeTab) ? 'text-blue-600' : ''">
                                        Ítems
                                    </span>
                                    <svg class="w-3.5 h-3.5 transition-transform duration-200"
                                        :class="dropdownItemsOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                ...
                                {{-- Dropdown panel --}}
                                <div x-show="dropdownItemsOpen" x-transition:enter="ease-out duration-150"
                ...
                <div class="max-w-7xl mx-auto px-6 py-6">
                    ...
                    <div x-show="activeTab === 'resumen'" x-cloak>
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                            ...
                            <div class="bg-white rounded-lg shadow-sm p-5 border border-slate-100">
                                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Sueldo Base
                                </p>
                                @if ($emp?->salary)
                            ...
                            <div class="bg-white rounded-lg shadow-sm p-5 border border-slate-100">
                                ...
                                @if ($diasEmpresa !== null)
                                    <p class="text-base font-black text-violet-600">
                                        {{ $aniosEmpresa > 0 ? $aniosEmpresa . ' año' . ($aniosEmpresa > 1 ? 's' : '') : $diasEmpresa . ' días' }}
                                    </p>
                                @else
                            ...
                            <div class="bg-white rounded-lg shadow-sm p-5 border border-slate-100">
                                ...
                                <p class="text-sm font-bold text-slate-800">
                                    {{ optional($emp?->afp)->nombre ?? 'No cotiza (Exento / Jubilado)' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                            ...
                            <div class="bg-white rounded-lg shadow-sm border border-slate-100 p-5 lg:col-span-2">
                                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Contrato y
                                    Remuneración</h3>
                            ...
                            <div class="bg-white rounded-lg shadow-sm border border-slate-100 p-5">
                                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Previsión
                                    Social</h3>
                                <div class="space-y-3">
                                    <div class="flex items-center justify-between py-2 border-b border-slate-50">
                                        <span class="text-xs font-medium text-slate-500">AFP</span>
                                        <span
                                            class="text-xs font-bold text-slate-800">{{ optional($emp?->afp)->nombre ?? '—' }}</span>
                                    </div>
                                    <div class="flex items-center justify-between py-2 border-b border-slate-50">
                                    ...
                                    <div class="flex items-center justify-between py-2">
                                        ...
                                        <span
                                            class="text-xs font-bold text-slate-800">{{ optional($emp?->ccaf)->nombre ?? '—' }}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div x-show="activeTab === 'perfil'" x-cloak>
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                            ...
                            {{-- Datos personales --}}
                            <div class="bg-white rounded-lg shadow-sm border border-slate-100 p-5 lg:col-span-2">
                                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-5">Datos
                                    Personales</h3>
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-5">
                                    ...
                                    @foreach ($fields as [$label, $value, $mono])
                                        <div>
                                            <dt class="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">
                                                {{ $label }}</dt>
                                            <dd
                                                class="mt-0.5 text-sm font-semibold text-slate-800 {{ $mono ? 'font-mono' : '' }}">
                                                {{ $value ?? '—' }}</dd>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="space-y-4">
                                <div class="bg-white rounded-lg shadow-sm border border-slate-100 p-5">
                                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Previsión y
                                        Salud</h3>
                                    <div class="space-y-3">
                                        ...
                                        <div
                                            class="p-3 rounded-lg {{ $emp?->health_system === 'isapre' ? 'bg-purple-50 border border-purple-100' : 'bg-teal-50 border border-teal-100' }}">
                                            <p
                                                class="text-[10px] font-bold {{ $emp?->health_system === 'isapre' ? 'text-purple-500' : 'text-teal-500' }} uppercase">
                                                Salud</p>
                                            <p
                                                class="text-sm font-bold {{ $emp?->health_system === 'isapre' ? 'text-purple-900' : 'text-teal-900' }} mt-0.5">
                                                @if ($emp?->health_system === 'isapre')
                                                    {{ optional($emp->isapre)->nombre ?? 'ISAPRE' }}
                                                    @if ($emp->health_contribution)
                                                        <span class="text-xs font-normal">({{ $emp->health_contribution }}
                                                            UF)</span>
                                                    @endif
                                                @else
                                                    {{ $healthLabels[$emp?->health_system ?? ''] ?? '—' }}
                                ...
                                {{-- Pago --}}
                                <div class="bg-white rounded-lg shadow-sm border border-slate-100 p-5">
                                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Medio de
                                        Pago</h3>
                                    <p class="text-sm font-bold text-slate-800">
                                        {{ $paymentLabels[$emp?->payment_method ?? ''] ?? '—' }}</p>
                                    @if ($emp?->payment_method === 'transferencia' && $emp?->bank_id)
                                        <div class="mt-3 space-y-1 text-xs text-slate-500">
                                            <p><span class="font-medium">Banco:</span>
                                                {{ optional($emp->bank)->name ?? '—' }}</p>
                                            <p><span class="font-medium">Cuenta:</span>
                                                {{ $emp->bank_account_type ? ucfirst($emp->bank_account_type) : '—' }}</p>
                    ...
                    <div x-show="activeTab === 'haberes'" x-cloak>
                        <div class="bg-white rounded-lg shadow-sm p-4 border border-slate-100">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <span
                                    class="text-xs bg-blue-50 text-blue-600 border border-blue-100 px-2.5 py-1 rounded-full font-medium">Próximamente</span>
                            </div>
                            <div class="py-12 flex flex-col items-center text-center">
                                ...
                                <p class="text-xs text-slate-400 mt-1 max-w-xs">Aquí aparecerán los bonos fijos,
                                    asignaciones especiales y otros haberes pactados.</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-4 border border-slate-100">
                            ...
                            <div class="py-12 flex flex-col items-center text-center">
                                <div class="w-14 h-14 rounded-xl bg-red-50 flex items-center justify-center mb-3">
                                    <svg class="w-7 h-7 text-red-300" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                ...
                                <p class="text-xs text-slate-400 mt-1 max-w-xs">Aquí aparecerán los descuentos fijos y
                                    retenciones pactadas con el colaborador.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'creditos'" x-cloak>
                        <div class="bg-white rounded-lg shadow-sm p-4 border border-slate-100">
                            ...
                            <div class="py-12 flex flex-col items-center text-center">
                                ...
                                <p class="text-xs text-slate-400 mt-1 max-w-xs">Aquí aparecerán los créditos sociales,
                                    préstamos de empresa y cuotas por descontar.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'liquidaciones'" x-cloak>
                        <div class="bg-white rounded-lg shadow-sm border border-slate-100">
                            ...
                            <div class="divide-y divide-slate-50">
                                @foreach ([['Marzo 2025', '$ 987.340', 'Pagada', 'bg-emerald-100 text-emerald-700'], ['Febrero 2025', '$ 987.340', 'Pagada', 'bg-emerald-100 text-emerald-700'], ['Enero 2025', '$ 982.100', 'Pagada', 'bg-emerald-100 text-emerald-700']] as [$mes, $monto, $estado, $badge])
                                    <div class="flex items-center gap-4 px-5 py-4 opacity-40">
                                        ...
                                        <div class="flex-1">
                                            <p class="text-sm font-semibold text-slate-700">Liquidación
                                                {{ $mes }}</p>
                                            <p class="text-xs text-slate-400">Líquido: {{ $monto }}</p>
                                        </div>
                                        <span
                                            class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $badge }}">{{ $estado }}</span>
                            ...
                            <div class="px-5 py-5 text-center">
                                <p class="text-xs text-slate-400">Las liquidaciones reales estarán disponibles al procesar
                                    la primera nómina.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'asistencia'" x-cloak>
                        <div class="bg-white rounded-lg shadow-sm p-4 border border-slate-100">
                            ...
                            <p class="text-center text-xs text-slate-400">El módulo de asistencia estará disponible
                                próximamente.</p>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'documentos'" x-cloak>
                        <div class="bg-white rounded-lg shadow-sm p-4 border border-slate-100">
                            <div class="flex items-center justify-between mb-5">
                                ...
                                <button
                                    class="text-xs font-semibold text-blue-600 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors flex items-center gap-1.5">
                            ...
                            <div class="grid grid-cols-4 md:grid-cols-6 gap-4 opacity-40">
                                @foreach (['Contrato.pdf', 'Anexo 1.pdf', 'CI Frente.jpg', 'CI Reverso.jpg', 'AFP Cert.pdf'] as $doc)
                                    <div
                                        class="flex flex-col items-center gap-2 p-3 rounded-xl border border-dashed border-slate-200 bg-slate-50 cursor-not-allowed">
                                        <svg class="w-9 h-9 text-red-300" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                        </svg>
                                        <span
                                            class="text-[10px] text-slate-500 font-medium text-center leading-tight">{{ $doc }}</span>
                                    </div>
                                @endforeach
                            </div>
                            <p class="text-center text-xs text-slate-400 mt-6">La gestión de documentos estará disponible
                                próximamente.</p>
                        </div>
                    </div>
                                ...
                                <div class="space-y-0">
                                    @php
                                        $events = [
                                            [
                                                'Hoy',
                                                'Perfil actualizado',
                                                'Se actualizó la información del colaborador.',
                                                'bg-blue-500',
                                            ...
                                            [
                                                ...
                                                optional($emp?->company)->name ?? 'Empresa',
                                                'bg-emerald-500',
                                                'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745',
                                            ],
                                            [
                                                $emp?->created_at?->format('d M Y') ?? '—',
                                                'Ficha creada en Remsis',
                                    ...
                                    @foreach ($events as [$date, $title, $desc, $color, $icon])
                                        <div class="relative flex gap-4 pb-7">
                                            <div
                                                class="absolute -left-6 top-0.5 w-8 h-8 rounded-full {{ $color }} flex items-center justify-center flex-shrink-0 z-10 ring-2 ring-white">
